...
def drawer(record):
    with ui.left_drawer().props('behavior="desktop"') as drawer:
        ...
        ui.label("Interval")
        ui.input(value=record.interval, on_change=lambda e: setattr(record, "interval", float(e.value)))
        ui.label("Width")
...
def main_page(record):
    ...
    def update_image():
        response = requests.get(f"{record.url}/capture")
    ...
    def start_timer():
        global timer
        requests.get(f"{record.url}/start")
    ...
    def stop_timer():
        global timer
        timer.cancel()
        requests.get(f"{record.url}/stop")
